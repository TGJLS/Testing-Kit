...
def ssh_deliver(base_url, headers, ssh_cfg, conn):
    host = ssh_cfg["host"]
    ...
    preamble = [r["command"] for r in _db.get_preamble(conn)]
    for cmd in preamble:
...
def run_tests(config_path: str, conn) -> dict:
    """Run all tasks from DB and return a structured result dict."""
    try:
        cfg = load_yaml(config_path)
    except FileNotFoundError as e:
        raise RuntimeError(f"Config file not found: {e.filename}")

    tasks = _db.get_tasks(conn)
    ...
    if ssh_cfg:
        ssh_client, agent_id = ssh_deliver(base_url, headers, ssh_cfg, conn)
    else:
    ...
    try:
        ...
        if ssh_client:
            if ssh_cfg.get("terminate", False):
                ...
                remove_agents_by_name(base_url, headers, _exe_name(ssh_cfg["agent_path"]))
            ssh_client.close()
